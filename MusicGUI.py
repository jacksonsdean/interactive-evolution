...
class GUI:
    ...
    def settings_window(self):
        ...
        def save_settings():
            needs_reset = False
            self.config.resize_train = [int(res_input.get("1.0", "end-1c")), int(res_input.get("1.0", "end-1c"))]
    ...
    def main_window(self):
        ...
        def load_music():
            file = filedialog.askopenfile(mode='r', filetypes=[('MP3', '*.mp3'), ('WAV', '*.wav')], initialdir="./music")
            music_label.config(text=f"Song: loading...")
            music_label.update()
            ...
            music_label.config(text=f"Song: {os.path.basename(file.name)}")
        ...
        music_label = tk.Label(self.command_panel, text='Song:', font=('Arial', 12, 'bold'))
        
        image = self.individual.get_image(1024, 1024, self.config.color_mode)
        img = Image.fromarray((image*255.0).astype(np.uint8))
        img_tk = ImageTk.PhotoImage(img)
        ...
        col+=1
        settings_btn.grid(row=row, column=col,sticky='NEWS')
        row+=1
        col = 0
        ...
        row+=1
        col = 0
        music_label.grid(row=row, column=col,sticky='NEWS')
        col=0
        row+=1
    ...
    def make_vis(self):
        ...
        def new_frame(time_index, last_time):
            ...
            # if(True):
                ...
                image = (self.individual.get_image(res[0], res[1], self.individual.config.color_mode, True) * 255.0).astype(np.uint8)
                prog_bar.step(1)
                images.append(image)
